...
#        print("PING!!", self.ipv4)
#        try:
#            print("Ping google.com: ", (wifi.radio.ping(ip=ipv4)*1000), "ms")
#        except TypeError:
#            print("Fail to ping.")
...
#SOS#            synth.set_chorus(3, self.chorus_level(), self.chorus_feedback(), 0)
    ...
    def get_score_files(self, init_list=[]):
        score_files = os.listdir('SYNTH/MUSIC/')
    ...
    def get_score_file_data(self, score_file_name):
        with open('SYNTH/MUSIC/' + score_file_name, 'r') as f:
            js = json.load(f)
        
        return js
                

    def get_score_files_list(self):
        with open('SYNTH/MUSIC/list.json', 'r') as f:
            js = json.load(f)
...
if __name__=='__main__':
    ...
    def base(request: Request, methods=[adafruit_httpserver.methods.GET, adafruit_httpserver.methods.POST]):
        ...
        try:
            ...
            score_list = instrument_guitar.get_score_files_list()
        ...
        try:
            ...
            if len(posts) > 0:
                ...
                if file_name != 'NEW':
                    ...
                    score_data = instrument_guitar.get_score_file_data(file_name)
                    score_list = instrument_guitar.get_score_files_list()
        ...
        try:
            ...
            if len(posts) > 0:
                ...
                for post in posts:
                    ...
                    if valvar[0] == 'SCORE_LIST':
                        score_list = decoded.strip().rstrip()

                if len(score_list) > 0:
                    status_message = 'ERROR: Guitar Scores List [JSON ERROR]'
                    js = json.loads(score_list)
                    print('GUITAR SCORES LIST JSON:', json.dumps(js))

                    status_message = 'ERROR: Guitar Scores List save error.'
                    with open('SYNTH/MUSIC/list.json', 'w') as f:
                        json.dump(js, f)
